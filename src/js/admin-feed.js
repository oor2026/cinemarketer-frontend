    ...
    async quitar() {
        ...
        async moverCarrusel(itemId, direccion) {
            ...
            async seleccionarPeliculaCarrusel(movieId) {
                ...
                // ==============================================
                // admin-feed.js — Gestión del feed de Series
                // Calcado 1:1 de adminFeed, con su propio endpoint base y sin la
                // opción de Ranking Trivia (Series tendrá el suyo propio más
                // adelante, no comparte el de Películas).
                // ==============================================
                const adminFeedSerie = {
                    ...
                    // ------------------------------------------
                    async cargarDestacadaActual() {
                        const cont = document.getElementById('feedDestacadaActualSerie');
                        cont.innerHTML = `<p style="color:#888;"><i class="fas fa-spinner fa-spin"></i> Cargando...</p>`;

                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/destacada`, {
                                headers: { 'Authorization': `Bearer ${token}` }
                            });

                            if (response.status === 204) {
                                cont.innerHTML = `<p style="color:#888;">No hay ninguna serie destacada configurada. La sección está oculta en el feed real.</p>`;
                                return;
                            }
                            if (!response.ok) throw new Error();

                            const data = await response.json();
                            const serie = await this.resolverSerie(data.seriesId);

                            cont.innerHTML = this.renderPreview(serie, data.updatedAt, data.updatedByAdminEmail);
                        } catch (error) {
                            cont.innerHTML = `<p style="color:#e50914;">Error al cargar la destacada actual</p>`;
                        }
                    },

                    renderPreview(serie, updatedAt, updatedByAdminEmail) {
                        if (!serie) {
                            return `<p style="color:#e50914;">La serie configurada ya no está disponible en TMDb</p>`;
                        }
                        const poster = serie.poster_path
                            ? `https://image.tmdb.org/t/p/w200${serie.poster_path}`
                            : 'assets/images/isologotipo.webp';
                        const anio = serie.first_air_date ? serie.first_air_date.substring(0, 4) : '—';
                        const meta = updatedAt
                            ? `<br><small style="color:#888;">Actualizada el ${new Date(updatedAt).toLocaleDateString('es-AR')} por ${updatedByAdminEmail || '—'}</small>`
                            : '';

                        return `
                            <img src="${poster}" alt="${serie.name}" style="width:70px;height:105px;object-fit:cover;border-radius:6px;">
                            <div>
                                <strong>${serie.name}</strong> <span style="color:#888;">(${anio})</span><br>
                                <span style="color:#f5c518;">★ ${serie.vote_average ? serie.vote_average.toFixed(1) : '—'}</span>
                                ${meta}
                            </div>
                            <button class="btn-accion btn-eliminar" style="margin-left:auto;" onclick="adminFeedSerie.quitar()" title="Quitar destacada">
                                <i class="fas fa-times"></i> Quitar
                            </button>
                        `;
                    },

                    async resolverSerie(seriesId) {
                        try {
                            const response = await fetch(`${CONFIG.API_URL}/series/${seriesId}`, {
                                headers: { 'Authorization': `Bearer ${token}` }
                            });
                            if (!response.ok) throw new Error();
                            return await response.json();
                    ...
                    buscar(texto) {
                        clearTimeout(this.debounceTimer);
                        const query = texto.trim();
                        const resultados = document.getElementById('feedResultadosBusquedaSerie');
                        ...
                        this.debounceTimer = setTimeout(async () => {
                            try {
                                const response = await fetch(`${CONFIG.API_URL}/series/search?query=${encodeURIComponent(query)}`, {
                                    headers: { 'Authorization': `Bearer ${token}` }
                                });
                                if (!response.ok) throw new Error();
                                const data = await response.json();
                    ...
                    renderResultados(series) {
                        const cont = document.getElementById('feedResultadosBusquedaSerie');
                        ...
                        cont.innerHTML = series.slice(0, 8).map(s => {
                            const poster = s.poster_path
                                ? `https://image.tmdb.org/t/p/w92${s.poster_path}`
                                : 'assets/images/isologotipo.webp';
                            const anio = s.first_air_date ? s.first_air_date.substring(0, 4) : '—';
                            return `
                                <div onclick="adminFeedSerie.seleccionar(${s.id})"
                                     style="display:flex;gap:0.75rem;align-items:center;padding:0.5rem 0.75rem;cursor:pointer;border-bottom:1px solid #f0f0f0;"
                                     onmouseover="this.style.background='#f8f8f8'" onmouseout="this.style.background='white'">
                                    <img src="${poster}" style="width:32px;height:48px;object-fit:cover;border-radius:4px;">
                                    <span>${s.name} <small style="color:#888;">(${anio})</small></span>
                                </div>`;
                        }).join('');
                        cont.style.display = 'block';
                    },

                    async seleccionar(seriesId) {
                        document.getElementById('feedResultadosBusquedaSerie').style.display = 'none';
                        document.getElementById('feedBuscarSerie').value = '';

                        const serie = await this.resolverSerie(seriesId);
                        if (!serie) {
                            alert('No se pudo cargar esa serie');
                            return;
                        }

                        this.serieSeleccionadaId = seriesId;
                        const preview = document.getElementById('feedSeleccionPreviewSerie');
                        preview.style.display = 'flex';
                        preview.innerHTML = this.renderPreview(serie, null, null)
                            .replace(/<button.*?<\/button>/s, '');

                        document.getElementById('feedBtnGuardarSerie').disabled = false;
                    },
                    ...
                    async guardar() {
                        ...
                        const btn = document.getElementById('feedBtnGuardarSerie');
                        btn.disabled = true;
                        btn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> Guardando...`;
                        ...
                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/destacada`, {
                                method: 'PUT',
                                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                                body: JSON.stringify({ seriesId: this.serieSeleccionadaId })
                            });
                            if (!response.ok) {
                                const err = await response.json().catch(() => ({}));
                                throw new Error(err.error || 'Error al guardar');
                            ...
                            document.getElementById('feedSeleccionPreviewSerie').style.display = 'none';
                            await this.cargarDestacadaActual();
                        } catch (error) {
                            alert(error.message || 'Error al guardar la destacada');
                        } finally {
                    ...
                    async quitar() {
                        if (!confirm('¿Quitar la serie destacada? El feed va a ocultar esa sección hasta que elijas otra.')) return;
                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/destacada`, {
                                method: 'DELETE',
                                headers: { 'Authorization': `Bearer ${token}` }
                    ...
                    async cargarCarrusel() {
                        const cont = document.getElementById('carruselListaActualSerie');
                        cont.innerHTML = `<p style="color:#888;"><i class="fas fa-spinner fa-spin"></i> Cargando...</p>`;

                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/carrusel`, {
                                headers: { 'Authorization': `Bearer ${token}` }
                            });
                            if (!response.ok) throw new Error();
                            const items = await response.json();
                    ...
                    async renderFilaCarrusel(item, idx, total) {
                        ...
                        if (item.tipo === 'SERIE_DESTACADA') {
                            ...
                            try {
                                ...
                                etiqueta = s ? `${s.name} <small style="color:#888;">(serie)</small>` : `Serie #${item.seriesId} (no encontrada)`;
                            } catch {
                                etiqueta = `Serie #${item.seriesId}`;
                            }
                        } else {
                            icono = item.tipo === 'PREMIO_COMUN' ? '<i class="fas fa-gift"></i>' : '<i class="fas fa-star"></i>';
                            const urlBase = item.tipo === 'PREMIO_COMUN' ? '/rewards/' : '/premium/rewards/';
                        ...
                        return `
                            <div style="display:flex;align-items:center;gap:0.75rem;padding:0.6rem 0.9rem;background:#f8f8f8;border-radius:8px;">
                                ${icono}
                                <span style="flex:1;">${etiqueta}</span>
                                <button class="btn-accion" ${idx === 0 ? 'disabled style="opacity:0.3;"' : ''} onclick="adminFeedSerie.moverCarrusel(${item.id}, 'subir')" title="Subir"><i class="fas fa-chevron-up"></i></button>
                                <button class="btn-accion" ${idx === total - 1 ? 'disabled style="opacity:0.3;"' : ''} onclick="adminFeedSerie.moverCarrusel(${item.id}, 'bajar')" title="Bajar"><i class="fas fa-chevron-down"></i></button>
                                <button class="btn-accion btn-eliminar" onclick="adminFeedSerie.quitarDelCarrusel(${item.id})" title="Quitar"><i class="fas fa-times"></i></button>
                            </div>`;
                    },
                    ...
                    async agregarSerieAlCarrusel() {
                        ...
                        document.getElementById('carruselBuscarPremioSerie').focus();
                        document.getElementById('carruselResultadosPremioSerie').style.display = 'none';
                    },

                    buscarPremio(texto) {
                        clearTimeout(this.debounceCarruselTimer);
                        const query = texto.trim().toLowerCase();
                        const cont = document.getElementById('carruselResultadosPremioSerie');
                        ...
                        this.debounceCarruselTimer = setTimeout(async () => {
                            try {
                                ...
                                cont.innerHTML = premios.slice(0, 15).map(p => `
                                    <div onclick="adminFeedSerie.seleccionarPremio(${p.id})"
                                         style="display:flex;gap:0.75rem;align-items:center;padding:0.5rem 0.75rem;cursor:pointer;border-bottom:1px solid #f0f0f0;"
                                         onmouseover="this.style.background='#f8f8f8'" onmouseout="this.style.background='white'">
                                        ${p.imageUrl ? `<img src="${p.imageUrl}" style="width:32px;height:32px;object-fit:cover;border-radius:4px;">` : '<i class="fas fa-gift" style="width:32px;text-align:center;color:#888;"></i>'}
                                        <span>${p.name}</span>
                    ...
                    async seleccionarPremio(rewardId) {
                        const tipo = this.tipoPremioSeleccionando === 'COMUN' ? 'PREMIO_COMUN' : 'PREMIO_ESPECIAL';
                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/carrusel/premio`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                                body: JSON.stringify({ tipo, rewardId })
                            });
                            ...
                            document.getElementById('carruselSelectorPremioSerie').style.display = 'none';
                            document.getElementById('carruselResultadosPremioSerie').style.display = 'none';
                            await this.cargarCarrusel();
                        } catch (error) {
                            alert(error.message);
                    ...
                    async quitarDelCarrusel(itemId) {
                        if (!confirm('¿Quitar este elemento del carrusel?')) return;
                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/carrusel/${itemId}`, {
                                method: 'DELETE',
                                headers: { 'Authorization': `Bearer ${token}` }
                            });
                            if (!response.ok) throw new Error();
                    ...
                    async moverCarrusel(itemId, direccion) {
                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/carrusel/${itemId}/${direccion}`, {
                                method: 'POST',
                                headers: { 'Authorization': `Bearer ${token}` }
                            });
                            if (!response.ok) throw new Error();
                    ...
                    debounceCarruselSerieTimer: null,

                    abrirSelectorSerie() {
                        document.getElementById('carruselSelectorSerie').style.display = 'block';
                        document.getElementById('carruselBuscarSerie').value = '';
                        document.getElementById('carruselBuscarSerie').focus();
                        document.getElementById('carruselResultadosSerie').style.display = 'none';
                    },

                    buscarSerieCarrusel(texto) {
                        clearTimeout(this.debounceCarruselSerieTimer);
                        const query = texto.trim();
                        const cont = document.getElementById('carruselResultadosSerie');
                        ...
                        this.debounceCarruselSerieTimer = setTimeout(async () => {
                            try {
                                const response = await fetch(`${CONFIG.API_URL}/series/search?query=${encodeURIComponent(query)}`, {
                                    headers: { 'Authorization': `Bearer ${token}` }
                                });
                                ...
                                cont.innerHTML = series.slice(0, 8).map(s => {
                                    ...
                                    const anio = s.first_air_date ? s.first_air_date.substring(0, 4) : '—';
                                    return `
                                        <div onclick="adminFeedSerie.seleccionarSerieCarrusel(${s.id})"
                                             style="display:flex;gap:0.75rem;align-items:center;padding:0.5rem 0.75rem;cursor:pointer;border-bottom:1px solid #f0f0f0;"
                                             onmouseover="this.style.background='#f8f8f8'" onmouseout="this.style.background='white'">
                                            <img src="${poster}" style="width:32px;height:48px;object-fit:cover;border-radius:4px;">
                                            <span>${s.name} <small style="color:#888;">(${anio})</small></span>
                    ...
                    async seleccionarSerieCarrusel(seriesId) {
                        try {
                            const response = await fetch(`${CONFIG.API_URL}/admin/series-feed/carrusel/serie-nueva`, {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                            ...
                            if (!response.ok) {
                                const err = await response.json().catch(() => ({}));
                                throw new Error(err.error || 'Error al agregar la serie');
                            }
                            document.getElementById('carruselSelectorSerie').style.display = 'none';
                            document.getElementById('carruselResultadosSerie').style.display = 'none';
                            await this.cargarCarrusel();
                        } catch (error) {
                            alert(error.message);
                ...
                window.adminFeedSerie = adminFeedSerie;
